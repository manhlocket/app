...
Biểu tượng trong các màn hình có ảnh tham chiếu được tách đúng pixel vào `assets/sprites/`; gói không chứa nguyên ảnh chụp màn hình. Bộ icon “iOS 26 by Kev” chưa được đưa vào mã nguồn vì tệp ZIP từ liên kết Dropbox chưa đọc được trong phiên này; các biểu tượng khác vẫn dùng SVG. Chạm mục để chuyển trang từ phải sang trái; vuốt từ mép trái sang phải hoặc chạm nút quay lại để về trang trước.
